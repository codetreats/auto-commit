            ...
            if (repo.changesCount > 0) {
                ...
                messageBox.innerHTML = `
                    <input
                        ...
                        value="${repo.suggestedCommitMessage}"
                    />
                `;

                const input = messageBox.querySelector('input');
                commitMessages[`${repo.category}/${repo.repo}`] = repo.suggestedCommitMessage;
...
async function commitChanges() {
    ...
    // Commit-Daten vorbereiten
    const commitList = reposWithChanges.map(repo => ({
        category: repo.category,
        repo: repo.repo,
        message: commitMessages[`${repo.category}/${repo.repo}`]
    }));

    const commits = { commits: commitList }
    ...
    try {
        ...
        alert('Fehler beim Committen: ' + error.message);
    }
}
